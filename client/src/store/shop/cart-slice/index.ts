...
export const addToCart = createAsyncThunk(
  ...
  async ({ userId, productId, quantity }: { userId: string; productId: string; quantity: number }) => {
    const response = await axios.post("https://ecommerce-store-backend-ten.vercel.app/api/shop/cart/add", {
      userId,
      productId,
      quantity,
    });
...
export const fetchCartItems = createAsyncThunk(
  "cart/fetchCartItems",
  async (userId: string) => {
    const response = await axios.get(`https://ecommerce-store-backend-ten.vercel.app/api/shop/cart/get/${userId}`);
    return response.data;
  }
);
...
export const deleteCartItem = createAsyncThunk(
  ...
  async ({ userId, productId }: { userId: string; productId: string }) => {
    const response = await axios.delete(`https://ecommerce-store-backend-ten.vercel.app/api/shop/cart/${userId}/${productId}`);
    return response.data;
  }
);
...
export const updateCartQuantity = createAsyncThunk(
  ...
  async ({ userId, productId, quantity }: { userId: string; productId: string; quantity: number }) => {
    const response = await axios.put("https://ecommerce-store-backend-ten.vercel.app/api/shop/cart/update-cart", {
      userId,
      productId,
      quantity,
    });
